...
class ReconstructionMetrics:
    ...
    def print_reconstruction_metrics(self, metrics: Dict[str, float], title: str = "Reconstruction Metrics"):
        ...
        # # 样本维度
        # logger.info("\nSample-wise Metrics (across samples):")
        # logger.info(f"  Pearson: {metrics.get('sample_wise_pearson_mean', 0):.6f} ± {metrics.get('sample_wise_pearson_std', 0):.6f}")
        # logger.info(f"  R²:      {metrics.get('sample_wise_r2_mean', 0):.6f} ± {metrics.get('sample_wise_r2_std', 0):.6f}")
        
        # # 特征维度
        # logger.info("\nFeature-wise Metrics (across features):")
        # logger.info(f"  Pearson: {metrics.get('feature_wise_pearson_mean', 0):.6f} ± {metrics.get('feature_wise_pearson_std', 0):.6f}")
        # logger.info(f"  R²:      {metrics.get('feature_wise_r2_mean', 0):.6f} ± {metrics.get('feature_wise_r2_std', 0):.6f}")
...
class CombinedMetrics:
    ...
    def compute_all_metrics(self, 
        ...
        Args:
            reconstruction_preds: 重建预测字典 {'rna': tensor, 'pheno': tensor}
            reconstruction_targets: 重建目标字典 {'rna': tensor, 'pheno': tensor}
            classification_pred: 分类预测
            classification_target: 分类目标
            classification_probs: 分类概率
            prefix: 指标前缀
            class_names: 类别名称
