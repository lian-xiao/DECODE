"""
多模态MOA预测模型的检索能力测试
支持直接使用融合特征进行MOA检索，无需二阶段训练
包含详细的检索指标计算和t-SNE可视化
"""
...
class MultiModalRetrievalTester:
    ...
    def _project_features_for_visualization(self, features: torch.Tensor, perplexity: int = 80) -> Optional[np.ndarray]:
        """在小样本 strict-OOD 场景下更稳健地投影到2D。"""
        features_np = features.cpu().numpy()
        n_samples, n_features = features_np.shape

        if n_samples < 2:
            logger.warning(f"Too few samples ({n_samples}) for visualization")
            return None

        if n_samples < 5:
            n_components = min(2, n_samples, n_features)
            projected = PCA(n_components=n_components, random_state=self.random_seed).fit_transform(features_np)
            if projected.shape[1] == 1:
                projected = np.concatenate([projected, np.zeros((n_samples, 1))], axis=1)
            return projected[:, :2]

        pca_components = min(50, n_samples, n_features)
        if pca_components < n_features:
            features_np = PCA(n_components=pca_components, random_state=self.random_seed).fit_transform(features_np)

        safe_perplexity = max(1, min(perplexity, n_samples - 1))
        if safe_perplexity >= n_samples:
            safe_perplexity = max(1, n_samples - 1)
    ...
    def create_tsne_visualization(self, features: torch.Tensor, labels: torch.Tensor, 
        ...
        """创建t-SNE可视化，重点显示指定的MOA类别"""
        logger.info(f"Creating t-SNE visualization for {scenario}...")
        
        # 转换为numpy
        labels_np = labels.cpu().numpy()
        
        features_2d = self._project_features_for_visualization(features, perplexity=perplexity)
    ...
    def create_scenario_comparison_visualization(self, all_features: Dict[str, torch.Tensor], 
        ...
        for i, scenario in enumerate(self.missing_scenarios):
            ...
            # 简化的t-SNE可视化
            labels_np = labels.cpu().numpy()
            features_2d = self._project_features_for_visualization(features, perplexity=50)
            if features_2d is None:
                continue
    ...
    def run_retrieval_test(self):
        ...
        features, moa_labels, metadata = self._remove_invalid_moa_samples_multimodal(
            features, moa_labels, metadata
        )
        features, moa_labels, metadata = self._remove_single_sample_moas_multimodal(
            features, moa_labels, metadata
        )
    ...
    def _remove_invalid_moa_samples_multimodal(
        self,
        features_dict: Dict[str, torch.Tensor],
        labels: torch.Tensor,
        metadata: List[Dict],
    ) -> Tuple[Dict[str, torch.Tensor], torch.Tensor, List[Dict]]:
        """移除无效或占位型MOA标签，避免 strict scaffold 评测被噪声标签主导。"""
        if labels is None or len(labels) == 0:
            return features_dict, labels, metadata

        invalid_tokens = {"", "nan", "none", "null", "unknown", "unk"}
        invalid_label_ids = set()

        for idx, moa_name in enumerate(self.moa_class_names):
            normalized = str(moa_name).strip().lower()
            if normalized in invalid_tokens or normalized.startswith("unknown"):
                invalid_label_ids.add(idx)

        if not invalid_label_ids:
            return features_dict, labels, metadata

        invalid_mask = torch.zeros(len(labels), dtype=torch.bool)
        for label_id in invalid_label_ids:
            invalid_mask |= (labels == label_id)

        removed_count = int(invalid_mask.sum().item())
        if removed_count == 0:
            return features_dict, labels, metadata

        logger.info(f"🧹 Removing {removed_count} samples with invalid MOA labels")
        valid_indices = torch.where(~invalid_mask)[0]

        filtered_features = {
            key: value[valid_indices]
    ...
    def generate_molecule_images(self):
        ...
        if not RDKIT_AVAILABLE:
            logger.warning("RDKit不可用，无法生成分子图片")
            return
        
        logger.info("🧬 生成所选分子的图片和SMILES指纹...")
...
def main():
    ...
    parser.add_argument('--data_loader_script', type=str,
                       help='Python script to create data loader')
    parser.add_argument('--output_dir', type=str,
                       default='results/multimodal_retrieval',
                       help='Output directory for results')
    parser.add_argument('--target_moas', nargs='+',
                       default=['Aurora kinase inhibitor', 'Eg5 inhibitor'],
                       help='Target MOAs to highlight')
    ...
    parser.add_argument('--missing_scenarios', nargs='+',
                       ...
                       help='Missing scenarios to test')
    ...
    parser.add_argument('--remove_drug_duplicates', default=True,
                       help='Remove duplicate drug samples based on drug features')
    parser.add_argument('--duplicate_threshold', type=float, default=1e-6,
                       help='Threshold for determining duplicate drugs (L2 distance)')
