...
class DisentangledVirtualScreeningModule(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.learning_rate = learning_rate
        self.dropout_rate = dropout_rate
        self.dose_values = [float(value) for value in dose_values]
        ...
        self.random_dose_range = random_dose_range
        ...
        self.disable_dose_conditioning = disable_dose_conditioning
        if disable_dose_conditioning:
            self.generator_model.disable_dose_conditioning = True
            self.fusion_model.disable_dose_conditioning = True
            logger.info("Dose conditioning disabled: gate bypassed on generator and fusion models")
        ...
        logger.info(f"  Dose values for averaging: {self.dose_values}")
        logger.info(f"  Learnable dose input: {self.learnable_dose_input}")
        logger.info(f"  Random dose range: {self.random_dose_range}")
        logger.info(f"  Generators frozen: {freeze_generators}")
        logger.info(f"  Molformer frozen: {freeze_molformer}")
        logger.info(f"  Dose conditioning disabled: {disable_dose_conditioning}")
    ...
    def _expand_dose_value(
        self,
        dose_value: torch.Tensor | float,
        batch_size: int,
        device: torch.device,
        dtype: torch.dtype,
    ) -> torch.Tensor:
        """将标量剂量扩展成 [batch_size, 1]。"""
        if isinstance(dose_value, torch.Tensor):
            scalar = dose_value.to(device=device, dtype=dtype).reshape(1, 1)
            return scalar.expand(batch_size, 1)
        return torch.full((batch_size, 1), float(dose_value), device=device, dtype=dtype)

    def _iter_dose_batches(
        self,
        batch_size: int,
        device: torch.device,
        dtype: torch.dtype,
        ...
        if self.learnable_dose_input and self.learnable_dose_values is not None:
            return [
                self._expand_dose_value(dose_value, batch_size, device, dtype)
                for dose_value in self.learnable_dose_values.unbind(0)
            ]
        if self.random_dose_range is not None and self.training:
            low, high = self.random_dose_range[0], self.random_dose_range[1]
            sampled = torch.empty(batch_size, 1, device=device, dtype=dtype).uniform_(low, high)
            return [sampled]
        return [
            self._expand_dose_value(dose_value, batch_size, device, dtype)
...
class LateFusionVirtualScreeningModule(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if disable_dose_conditioning:
            self.generator_model.disable_dose_conditioning = True
            logger.info("Dose conditioning disabled: gate bypassed on generator model")
...
class DisentangledVirtualScreeningModule(pl.LightningModule):
    ...
    def _generate_simulated_modalities(self, drug_features: torch.Tensor, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        all_simulated_rna = []
        all_simulated_pheno = []
        dose_batches = self._iter_dose_batches(
            batch_size=batch_size,
            device=device,
            dtype=drug_features.dtype,
        )
        
        with torch.no_grad():  # 生成器模型完全冻结
            for dose_batch in dose_batches:
                # 构建输入数据
                batch_data = {
                    'drug': drug_features,
                    'dose': dose_batch,
...
class SimplifiedDisentangledVirtualScreeningModule(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.disable_dose_conditioning = disable_dose_conditioning
        if disable_dose_conditioning:
            self.disentangled_model.disable_dose_conditioning = True
            logger.info("Dose conditioning disabled: gate bypassed on disentangled model")
    ...
    def extract_classifier_features(self, smiles_list: List[str], cached_features: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # 1. 获取Molformer特征（优先使用缓存）
        drug_features = self.extract_features(smiles_list, cached_features).to(device)
    ...
    def forward(self, smiles_batch: List[str], cached_features: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # 1. 获取Molformer特征（优先使用缓存）
        drug_features = self.extract_features(smiles_batch, cached_features).to(device)
    ...
    def _extract_disentangled_features(self, drug_features: torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        disentangled_forward_ctx = (
            torch.enable_grad()
            if (self.learnable_dose_input or not self.freeze_disentangled_model)
            else torch.no_grad()
        )
        
        with disentangled_forward_ctx:
            for dose_batch in dose_batches:
